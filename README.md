...
- **Robust Parsing:** Uses `pdfplumber` and regular expressions to reliably extract billing information, even if the bill's layout changes slightly.
- **Custom Calculations:** Implements the specific cost-sharing and adjustment logic from the original script.
- **WhatsApp Notifications:** Sends a formatted bill summary to multiple recipients using the Twilio API for WhatsApp.
...
- A Twilio account with a phone number capable of sending WhatsApp messages.
...
**Twilio Credentials:**
- `TWILIO_ACCOUNT_SID`: Your Twilio Account SID, found on your Twilio console dashboard.
- `TWILIO_AUTH_TOKEN`: Your Twilio Auth Token.
- `TWILIO_PHONE_NUMBER`: Your Twilio phone number that is WhatsApp-enabled (in E.164 format, e.g., `+14155238886`).

**Recipient Phone Numbers:**
- `RECIPIENT_PHONE_NUMBERS`: A comma-separated list of phone numbers to send the notification to. Numbers must be in E.164 format and include the `whatsapp:` prefix required by Twilio (e.g., `+15551234567,+15559876543`).
...
```
*(Note: You will need a library like `python-dotenv` to automatically load a `.env` file if running locally. This is not included in `requirements.txt` as it's not needed for deployment.)*
